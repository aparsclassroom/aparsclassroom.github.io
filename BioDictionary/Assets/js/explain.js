...
window.addEventListener("load", function() {
    ...
    fetch(e + t + n).then(e => e.json()).then(e => {
        ...
        b.addEventListener("click", function(e) {
            var n = { sl: t + "_" + i + "_" + o, time: new Date, name: l, url: window.location.href };
            ...
            for (var e = window.location.href, n = JSON.parse(localStorage.getItem("bookmarks")), t = 0; t < n.length; t++) n[t].url == e && (n.splice(t, 1), u.style.display = "none", b.style.display = "inline-block");
            localStorage.setItem("bookmarks", JSON.stringify(n)), y(), h()
        }), $("#Video").on("hidden.bs.modal", function() { player.currentTime = 0, player.pause() }), "" == g || "https://gdurl.com" == g ? (document.getElementById("image").style.display = "none", document.getElementById("fi").style.display = "none") : document.getElementById("image").style.cssText = "display:inline-block;", "" == m || "https://gdurl.com" == m ? document.getElementById("ad").style.display = "none" : document.getElementById("adImg").innerHTML = '\n                <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-volume-off-fill" viewBox="0 0 16 16">\n                <path d="M10.717 3.55A.5.5 0 0 1 11 4v8a.5.5 0 0 1-.812.39L7.825 10.5H5.5A.5.5 0 0 1 5 10V6a.5.5 0 0 1 .5-.5h2.325l2.363-1.89a.5.5 0 0 1 .529-.06z"/>\n              </svg>\n                ';
        b = document.getElementById("book"), u = document.getElementById("deletebook");
        var v = JSON.parse(localStorage.getItem("bookmarks"));
        if (null !== v) {
            if (v.filter(function(e) { return window.location.href === e.url }).length > 0) return u.style.display = "inline-block", b.style.display = "none", !1;
            u.style.display = "none", b.style.display = "inline-block", null === localStorage.getItem("bookmarks") || "[]" === localStorage.getItem("bookmarks") || h(), y()
        }

        function h() { for (var e = window.location.href, n = JSON.parse(localStorage.getItem("bookmarks")), t = 0; t < n.length; t++) return n[t].url === e ? b.style.display = "inline-block" : u.style.display = "none" }

        function y(e) { e = window.location.href; for (var n = JSON.parse(localStorage.getItem("bookmarks")), t = 0; t < n.length; t++) n[t].url === e ? (u.style.display = "inline-block", b.style.display = "none") : (u.style.display = "none", b.style.display = "inline-block") }
