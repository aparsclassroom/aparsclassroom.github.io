...
if (document.getElementById("stds")) {
  const countUps = new CountUp(
    "stds",
    document.getElementById("stds").getAttribute("countTo"),
  );
  if (!countUps.error) {
    countUps.start();
  } else {
...
firebase.auth().onAuthStateChanged(function (e) {
  if (e) {
    ...
    fetch(
      `https://${shopName2}/${productCode}/purchase/${Cycle}`,
      requestOptions,
    )
      .then((response) => {
      ...
      .then((result) => {
        if (result.status === 200) {
          ...
          form.addEventListener("submit", (em) => {
            ...
            fetch(
              `https://${shopName2}/${Cycle}/${productCode}/init`,
              requestOptions,
            )
              .then((response) => {
    ...
    (document.getElementById("app").style.display = "none"),
      ...
      document.getElementById("moda").addEventListener("click", () => {
        ...
        location.href =
          "/shop/dashboard/login?&signInSuccessUrl=" + location.pathname;
      });
