          ...
          if (content.type != "yt" || content.type != "ftp") {
            if (content.type == "pdf") {
              document.getElementById("content").src =
                `https://drive.google.com/file/d/${content.link.split("/")[5]}/preview`;
            } else {
              if (content.type == "quiz") {
                document.getElementById("content").src = content.link;
                document.getElementById("description").innerHTML =
                  `${content.description}<br> কুইজটি দেখতে সমস‍্যা হলে এই লিংকে ক্লিক করো <i class="fa-solid fa-hand-point-right"></i>&nbsp;<a href="${content.link}" target="_blank">${content.link}</a>`;
              } else {
                document.getElementById("content").src = content.link;
              }
            }
