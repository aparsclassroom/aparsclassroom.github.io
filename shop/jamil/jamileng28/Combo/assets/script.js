...
const jamilEng28ComboBlocks = {
    "819": {
        combos: [],
        cycles: [
            { cycle: "Cycle-1", products: ["817"] },
            { cycle: "Cycle-2", products: ["818"] }
        ]
    }
};

let jamilEng28EligibilityChecked = false;

function jamilEng28CheckPurchase(products, uid, cycle) {
...
function jamilEng28FindExistingPurchase(results) {
    return results
...
function jamilEng28InvoiceFromPurchase(result) {
    return result?.invoices?.[0]?.invoice || result?.Invoice;
}

function jamilEng28ShowExistingPurchase(result) {
    swal({
        title: "Already Enrolled !",
        icon: "success",
        button: "View Informations"
    }).then(() => {
        const invoice = jamilEng28InvoiceFromPurchase(result);
        if (invoice) {
...
function jamilEng28ValidateComboEligibility(uid) {
    const blockConfig = jamilEng28ComboBlocks[productCode];
    if (!blockConfig) {
    ...
    if (blockConfig.combos.length) {
        purchaseChecks.push(jamilEng28CheckPurchase(blockConfig.combos, uid));
    }
    blockConfig.cycles.forEach(check => {
        purchaseChecks.push(jamilEng28CheckPurchase(check.products, uid, check.cycle));
    });

    return Promise.allSettled(purchaseChecks).then(jamilEng28FindExistingPurchase);
}

document.addEventListener('submit', function(event) {
    if (!event.target || event.target.name !== 'purchase' || jamilEng28EligibilityChecked) {
        return;
    }
    ...
    jamilEng28ValidateComboEligibility(uid)
        .then(existingPurchase => {
            if (existingPurchase) {
                jamilEng28ShowExistingPurchase(existingPurchase);
                return;
            }

            jamilEng28EligibilityChecked = true;
...
firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        ...
        const jamilEng28BlockConfig = jamilEng28ComboBlocks[productCode] || { combos: [], cycles: [] };
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
        var raw = JSON.stringify({
            "products": [productCode].concat(jamilEng28BlockConfig.combos),
            'uid': e.uid
        });
        ...
        const purchaseChecks = [
            ...
            ...jamilEng28BlockConfig.cycles.map(check => {
                return jamilEng28CheckPurchase(check.products, e.uid, check.cycle);
            })
        ];

        Promise.allSettled(purchaseChecks)
            .then(result => {
                result = jamilEng28FindExistingPurchase(result) || { status: 404 };
