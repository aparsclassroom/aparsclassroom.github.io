...
// The 500৳ discount is already applied from the backend (pls / pls2), so just show the matching coupon as applied
function updateAutoCoupon() {
    document.getElementById('autoCouponCode').innerText =
        document.getElementById('addBooks').checked ? '"FRB27WBOOKS"' : '"FRB27"';
}
...
document.getElementById('addBooks').addEventListener('change', function () {
    ...
    if (isShipping) {
        ...
        Swal.fire({
            ...
            `,
            imageUrl: 'https://i.postimg.cc/5NXsmCCc/compressed-image.jpg',
