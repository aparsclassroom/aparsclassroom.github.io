...
function b2b28CheckPurchase(products, uid, cycle) {
...
function b2b28FindExistingPurchase(results) {
    return results
...
firebase.auth().onAuthStateChanged(function (e) {
    if (e) {
        ...
        const blockedComboProducts = b2b28CycleComboBlocks[Cycle] || [];
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
        ...
        if (blockedComboProducts.length) {
            purchaseChecks.push(b2b28CheckPurchase(blockedComboProducts, e.uid));
        }

        Promise.allSettled(purchaseChecks)
        .then((result) => {
            result = b2b28FindExistingPurchase(result) || { status: 404 };
