...
function applyBooksState(isShipping) {
    ...
    if (couponHint) { couponHint.textContent = isShipping ? 'SUPER27BOOK' : 'SUPER27'; couponHint.dataset.coupon = couponHint.textContent; }
...
if (urlProductCode === productCode) {
    // If URL has productCode (852), start with books unchecked and lock the choice
    applyBooksState(false);
    document.getElementById('addbooksdiv').style.display = 'none';
} else if (urlProductCode === productCode2) {
    // If URL has productCode2 (853), start with books checked and lock the choice
...
document.getElementById('addBooks').addEventListener('change', function () {
    ...
    Swal.fire({
        ...
        html: `
                <div>
                    <p>DMC DREAMERS BATCH-এর সঙ্গে বই নিতে চাইলে with Books অপশনটি বেছে নাও।</p>
...
firebase.auth().onAuthStateChanged(function (e) {
    if (e) {
        ...
        document.getElementById("app").addEventListener('click', () => {
            document.getElementById("app").style.display = "none", document.getElementById("cup").style.display = "block", document.getElementById("coupnbosh").style.display = "flex"
        })
