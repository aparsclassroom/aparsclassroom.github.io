...
const galacticos3PurchaseBlocks = {
    "800": {
        cycleProducts: [
            { cycle: "Cycle-1", products: ["800"] },
            { cycle: "Cycle-4", products: ["803"] },
            { cycle: "Cycle-5", products: ["804"] }
        ]
    },
    "801": {
        cycleProducts: [
            { cycle: "Cycle-2", products: ["801"] },
            { cycle: "Cycle-4", products: ["803"] },
            { cycle: "Cycle-5", products: ["804"] }
        ]
    },
...
function galacticos3PurchaseUrl(cycle) {
    return cycle
        ? `https://${shopName2}/v3/purchase/multiple/${cycle}`
        : `https://${shopName2}/v3/purchase/multiple`;
}

function galacticos3CheckPurchase(products, uid, cycle) {
    var myHeaders = new Headers();
    myHeaders.append("Content-Type", "application/json");

    return fetch(galacticos3PurchaseUrl(cycle), {
        method: 'POST',
        headers: myHeaders,
        body: JSON.stringify({ products, uid }),
        redirect: 'follow'
    }).then(response => response.json());
}

function galacticos3FindExistingPurchase(results) {
    return results
...
function galacticos3InvoiceFromPurchase(result) {
    return result?.invoices?.[0]?.invoice || result?.Invoice;
}

function galacticos3ShowExistingPurchase(result) {
    swal({
        title: "Already Enrolled !",
        icon: "success",
        button: "View Informations"
    }).then(() => {
        const invoice = galacticos3InvoiceFromPurchase(result);
        if (invoice) {
...
function galacticos3PurchaseChecks(uid) {
    const blockConfig = galacticos3PurchaseBlocks[productCode] || { cycleProducts: [{ cycle: Cycle, products: [productCode] }] };
    const checks = [];

    blockConfig.cycleProducts.forEach(check => {
        checks.push(galacticos3CheckPurchase(check.products, uid, check.cycle));
    });

    return checks;
}

function galacticos3InitPurchase(requestOptions) {
    return fetch(`https://${shopName2}/${Cycle}/${productCode}/init`, requestOptions);
}

function galacticos3AttachPurchaseSubmit(uid) {
    ...
    form.addEventListener('submit', em => {
        ...
        galacticos3InitPurchase(requestOptions)
...
firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        ...
        Promise.allSettled(galacticos3PurchaseChecks(e.uid))
            .then(result => {
                result = galacticos3FindExistingPurchase(result) || { status: 404 };
                if (result.status === 200) {
                    galacticos3ShowExistingPurchase(result);
                } else {
                    galacticos3AttachPurchaseSubmit(e.uid);
                }
            }).catch(() => {
                galacticos3AttachPurchaseSubmit(e.uid);
            });
        ...
        document.getElementById("app").addEventListener("click", e => {
            e.preventDefault();
            document.location.href = "/shop/dashboard/login?&signInSuccessUrl=" + location.pathname;
        });
    }
});
