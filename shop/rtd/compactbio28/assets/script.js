...
const compactbio28CycleComboBlocks = {
...
function compactbio28CheckPurchase(products, uid, cycle) {
...
function compactbio28FindExistingPurchase(results) {
    return results
...
firebase.auth().onAuthStateChanged(function (e) {
    if (e) {
        ...
        const blockedComboProducts = compactbio28CycleComboBlocks[Cycle] || [];
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
        ...
        if (blockedComboProducts.length) {
            purchaseChecks.push(compactbio28CheckPurchase(blockedComboProducts, e.uid));
        }

        Promise.allSettled(purchaseChecks)
        .then((result) => {
            result = compactbio28FindExistingPurchase(result) || { status: 404 };
