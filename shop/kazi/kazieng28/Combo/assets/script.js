...
const kaziEng28ComboBlocks = {
    "816": {
        combos: [],
        cycles: [
            { cycle: "Cycle-1", products: ["814"] },
            { cycle: "Cycle-2", products: ["815"] }
        ]
    }
};

let kaziEng28EligibilityChecked = false;

function kaziEng28CheckPurchase(products, uid, cycle) {
...
function kaziEng28FindExistingPurchase(results) {
    return results
...
function kaziEng28InvoiceFromPurchase(result) {
    return result?.invoices?.[0]?.invoice || result?.Invoice;
}

function kaziEng28ShowExistingPurchase(result) {
    swal({
        title: "Already Enrolled !",
        icon: "success",
        button: "View Informations"
    }).then(() => {
        const invoice = kaziEng28InvoiceFromPurchase(result);
        if (invoice) {
...
function kaziEng28ValidateComboEligibility(uid) {
    const blockConfig = kaziEng28ComboBlocks[productCode];
    if (!blockConfig) {
    ...
    if (blockConfig.combos.length) {
        purchaseChecks.push(kaziEng28CheckPurchase(blockConfig.combos, uid));
    }
    blockConfig.cycles.forEach(check => {
        purchaseChecks.push(kaziEng28CheckPurchase(check.products, uid, check.cycle));
    });

    return Promise.allSettled(purchaseChecks).then(kaziEng28FindExistingPurchase);
}

document.addEventListener('submit', function(event) {
    if (!event.target || event.target.name !== 'purchase' || kaziEng28EligibilityChecked) {
        return;
    }
    ...
    kaziEng28ValidateComboEligibility(uid)
        .then(existingPurchase => {
            if (existingPurchase) {
                kaziEng28ShowExistingPurchase(existingPurchase);
                return;
            }

            kaziEng28EligibilityChecked = true;
...
firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        ...
        const kaziEng28BlockConfig = kaziEng28ComboBlocks[productCode] || { combos: [], cycles: [] };
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
        var raw = JSON.stringify({
            "products": [productCode].concat(kaziEng28BlockConfig.combos),
            'uid': e.uid
        });
        ...
        const purchaseChecks = [
            ...
            ...kaziEng28BlockConfig.cycles.map(check => {
                return kaziEng28CheckPurchase(check.products, e.uid, check.cycle);
            })
        ];

        Promise.allSettled(purchaseChecks)
            .then(result => {
                result = kaziEng28FindExistingPurchase(result) || { status: 404 };
