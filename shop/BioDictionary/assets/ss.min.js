...
form.addEventListener("submit", e => { e.preventDefault(); const mm = product + "-tranx"; const tn = document.getElementById('Tranx');
    localStorage.setItem(mm, tn.value);
    fetch(scriptURL, { method: "POST", body: new FormData(form) }).then(e => alert("You've been successfully submitted your purchase request 🥰 \n Plz Wait for a manual Verification\n\nMeanwhile you can track your purchase status in Realtime!!\nClick Ok 🤩")).then(e => window.location.replace("https://aparsclassroom.com/shop/live-tracker.html")).catch(e => alert("Error!", e.message)) }), firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        var t = e.phoneNumber;
