...
form.addEventListener("submit", e => { e.preventDefault(), fetch(scriptURL, { method: "POST", body: new FormData(form) }).then(e => alert("You've been successfully submitted your purchase request 🥰 \n Plz Wait for a manual Verification\n\nMeanwhile you can track your purchase status in Realtime!!\nClick Ok 🤩")).then(e => window.location.replace("https://aparsclassroom.com/shop/live-tracker.html")).catch(e => console.error("Error!", e.message)) }), firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        var t = e.phoneNumber;
