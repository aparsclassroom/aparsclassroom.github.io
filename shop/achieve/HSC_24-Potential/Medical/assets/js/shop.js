const scriptURL =
  "https://script.google.com/macros/s/AKfycbwB5pP5582wlbWSjdu8Dp0Xy6WjFipKvtUmme4FowgwnXxzCyEyCLkXt0vRrdU4lJHcgw/exec";
...
firebase.auth().onAuthStateChanged(function (e) {
  if (e) {
    const branchApi =
      "https://crm.aparsclassroom.com/branch/find/available-branches?productId=" +
      productCode;
    ...
    $("#branch").on("change", function () {
      ...
      document.getElementById("branchInfo").innerHTML = `
                ...
                <img src="${$(this).find(":selected").data("photo")}" width="100%">
                        ...
                        `;
      const batchApi =
        "https://crm.aparsclassroom.com/branch/find/available-batches-pre-book?branchId=" +
        branchId +
        "&productId=" +
        productCode;
        ...
        .then((options) => {
          ...
          if (options.status == 200) {
            ...
            options.batchList.forEach((batch) => {
              $("#batch").append(
                `<option value="${batch.id}" data-time="${batch.time}">${batch.text}</option>`,
              );
            });
          }
        })
      ...
      .then((dashboard) => {
        if (dashboard.code != 200) {
          document
            ...
            .setAttribute("countTo", dashboard.enrolled.enrolled);
          ...
          swal({
            ...
            text:
              "Hello " +
              dashboard.message.username +
              "\nYour Booking Number : " +
              dashboard.message.Serial +
              "\nTime : " +
