document.getElementById("email").addEventListener("input", function (event) {
  if (document.getElementById("email").validity.typeMismatch) {
    document
      ...
      .setCustomValidity("We are expecting an e-mail address!");
  } else {
...
document.getElementById("phone").addEventListener("input", function (event) {
  if (document.getElementById("phone").validity.patternMismatch) {
    document
      ...
      .setCustomValidity(
        "Please enter a valid phone number (+8801XX XXX XXXX)!",
      );
  } else {
...
firebase.auth().onAuthStateChanged(function (e) {
  if (e) {
    const branchApi =
      "https://crm.apars.shop/branch/find/available-branches?productId=" +
      productCode;
      ...
      .then((options) => {
        if (options.status == 200) {
          options.branchList.forEach((branch) => {
            $("#branch").append(
              `<option value="${branch.text}" data-id="${branch.id}" data-address="${branch.address}" data-photo="${branch.photo}">${branch.text}</option>`,
            );
          });
        } else {
          swal({
    ...
    $("#branch").on("change", function () {
      ...
      document.getElementById("branchInfo").innerHTML = `
                ...
                <img src="${$(this).find(":selected").data("photo")}" width="100%">
                        ...
                        `;
      const batchApi =
        "https://crm.apars.shop/branch/find/available-batches?branchId=" +
        branchId +
        "&productId=" +
        productCode;
          ...
          if (options.status == 200) {
            ...
            options.batchList.forEach((batch) => {
              if (batch.disabled == false) {
                $("#batch").append(
                  `<option value="${batch.id}" data-time="${batch.time}">${batch.text} (${batch.students} / ${batch.max})</option>`,
                );
              } else {
                $("#batch").append(
                  `<option value="${batch.id}" data-time="${batch.time}" disabled>${batch.text} (Batch Full)</option>`,
                );
              }
            });
          }
        ...
        if (result.status === 200) {
          ...
          form.addEventListener("submit", (em) => {
            ...
            fetch(`https://${shopName2}/v2/${productCode}/init`, requestOptions)
              ...
              .then((result) => {
                if (
                  result != '{"status":404,"message":"Product Error"}' ||
                  result.status != 420
                ) {
    ...
    firebase
      ...
      .then((idTokenResult) => {
        ...
        if (!claims.roll) {
          fetch(
            `https://profile.aparsclassroom.com/profile/eligibility?uid=${firebase.auth().currentUser.uid}`,
          )
    ...
    (document.getElementById("app").style.display = "none"),
      ...
      document.getElementById("moda").addEventListener("click", () => {
        location.href =
          "/shop/dashboard/login?signInSuccessUrl=" +
...
var cupon,
  cpn = document.getElementById("cpnCheck");
...
function suc() {
  "" === document.getElementById("cupon").value
    ? (document.getElementById("cpnCheck").disabled = !0)
    : (document.getElementById("cpnCheck").disabled = !1);
}
    ...
    .then((loadedData) => {
      if (loadedData.status === "success") {
        ...
        var percent = Math.round(
          ((parseInt(loadedData.Off) + (fix - pls)) / fix) * 100,
        );
        document.getElementById("how").style.display = "block";
        document.getElementById("how").innerHTML =
          `<span style="color:red;">${percent}%</span> discounted by <span style="color:blue;">"${loadedData.Cupon}"</span> promo code`;
        document.getElementById("smp").innerHTML =
          "<del style='color:red'> " +
          fix +
          "৳</del> " +
          " <span style='color:rgb(26, 185, 66);;'>" +
          nes +
          " ৳</span>";
        document.getElementById("cup").style.display = "block";
        return;
