...
const fahadBio28ComboBlocks = {
    "731": {
        combos: ["733"],
        cycles: [
            { cycle: "Cycle-1", products: ["725"] },
            { cycle: "Cycle-3", products: ["727"] },
            { cycle: "Cycle-5", products: ["729"] }
        ]
    },
    "732": {
        combos: ["733"],
        cycles: [
            { cycle: "Cycle-2", products: ["726"] },
            { cycle: "Cycle-4", products: ["728"] },
            { cycle: "Cycle-6", products: ["730"] }
        ]
    },
    "733": {
        combos: ["731", "732"],
...
let fahadBio28EligibilityChecked = false;

function fahadBio28CheckPurchase(products, uid, cycle) {
...
function fahadBio28FindExistingPurchase(results) {
    return results
...
function fahadBio28InvoiceFromPurchase(result) {
    return result?.invoices?.[0]?.invoice || result?.Invoice;
}

function fahadBio28ShowExistingPurchase(result) {
    swal({
        title: "Already Enrolled !",
        icon: "success",
        button: "View Informations"
    }).then(() => {
        const invoice = fahadBio28InvoiceFromPurchase(result);
        if (invoice) {
...
function fahadBio28ValidateComboEligibility(uid) {
    const blockConfig = fahadBio28ComboBlocks[productCode];
    if (!blockConfig) {
    ...
    if (blockConfig.combos.length) {
        purchaseChecks.push(fahadBio28CheckPurchase(blockConfig.combos, uid));
    }
    blockConfig.cycles.forEach(check => {
        purchaseChecks.push(fahadBio28CheckPurchase(check.products, uid, check.cycle));
    });

    return Promise.allSettled(purchaseChecks).then(fahadBio28FindExistingPurchase);
}

document.addEventListener('submit', function(event) {
    if (!event.target || event.target.name !== 'purchase' || fahadBio28EligibilityChecked) {
        return;
    }
    ...
    fahadBio28ValidateComboEligibility(uid)
        .then(existingPurchase => {
            if (existingPurchase) {
                fahadBio28ShowExistingPurchase(existingPurchase);
                return;
            }

            fahadBio28EligibilityChecked = true;
...
firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        ...
        const fahadBio28BlockConfig = fahadBio28ComboBlocks[productCode] || { combos: [], cycles: [] };
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
        var raw = JSON.stringify({
            "products": [productCode].concat(fahadBio28BlockConfig.combos),
            'uid': e.uid
        });
        ...
        const purchaseChecks = [
            ...
            ...fahadBio28BlockConfig.cycles.map(check => {
                return fahadBio28CheckPurchase(check.products, e.uid, check.cycle);
            })
        ];

        Promise.allSettled(purchaseChecks)
            .then(result => {
                result = fahadBio28FindExistingPurchase(result) || { status: 404 };
