...
const lodestarEng28ComboBlocks = {
    "809": {
        combos: [],
        cycles: [
            { cycle: "Cycle-1", products: ["807"] },
            { cycle: "Cycle-2", products: ["808"] }
        ]
    }
};

let lodestarEng28EligibilityChecked = false;

function lodestarEng28CheckPurchase(products, uid, cycle) {
...
function lodestarEng28FindExistingPurchase(results) {
    return results
...
function lodestarEng28InvoiceFromPurchase(result) {
    return result?.invoices?.[0]?.invoice || result?.Invoice;
}

function lodestarEng28ShowExistingPurchase(result) {
    swal({
        title: "Already Enrolled !",
        icon: "success",
        button: "View Informations"
    }).then(() => {
        const invoice = lodestarEng28InvoiceFromPurchase(result);
        if (invoice) {
...
function lodestarEng28ValidateComboEligibility(uid) {
    const blockConfig = lodestarEng28ComboBlocks[productCode];
    if (!blockConfig) {
    ...
    if (blockConfig.combos.length) {
        purchaseChecks.push(lodestarEng28CheckPurchase(blockConfig.combos, uid));
    }
    blockConfig.cycles.forEach(check => {
        purchaseChecks.push(lodestarEng28CheckPurchase(check.products, uid, check.cycle));
    });

    return Promise.allSettled(purchaseChecks).then(lodestarEng28FindExistingPurchase);
}

document.addEventListener('submit', function(event) {
    if (!event.target || event.target.name !== 'purchase' || lodestarEng28EligibilityChecked) {
        return;
    }
    ...
    lodestarEng28ValidateComboEligibility(uid)
        .then(existingPurchase => {
            if (existingPurchase) {
                lodestarEng28ShowExistingPurchase(existingPurchase);
                return;
            }

            lodestarEng28EligibilityChecked = true;
...
firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        ...
        const lodestarEng28BlockConfig = lodestarEng28ComboBlocks[productCode] || { combos: [], cycles: [] };
        var myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
        var raw = JSON.stringify({
            "products": [productCode].concat(lodestarEng28BlockConfig.combos),
            'uid': e.uid
        });
        ...
        const purchaseChecks = [
            ...
            ...lodestarEng28BlockConfig.cycles.map(check => {
                return lodestarEng28CheckPurchase(check.products, e.uid, check.cycle);
            })
        ];

        Promise.allSettled(purchaseChecks)
            .then(result => {
                result = lodestarEng28FindExistingPurchase(result) || { status: 404 };
                if (result.status === 200) {
                    ...
                    form.addEventListener('submit', em => {
                        ...
                        fetch(`https://${shopName2}/836/init`, requestOptions)
                            .then(response => {
        ...
        কম্বোতে এনরোল করো <i class="fas fa-arrow-right"></i>
        `;
    } else {
