...
// Variables to track which product to buy and price
let activeProductCode = productCode;
let activePrice = pls;
...
// Function to check previous purchases
async function checkPreviousPurchases(uid) {
    try {
        const response = await fetch('https://shop.aparsclassroom.com/logs', {
            method: 'POST',
            headers: {
        ...
        if (data.status === 200 && data.info && Array.isArray(data.info)) {
            // if student has purchased "hbio27" or "hbio27wbook" with VALID/VALIDATED status
            const hasPreviousPurchase = data.info.find(item => 
                (item.status === "VALID" || item.status === "VALIDATED") &&
                item.Product && 
                (item.Product.productName === "hbio27" || item.Product.productName === "hbio27wbook")
            );
            
            if (hasPreviousPurchase) {
                activeProductCode = productCode2;
                activePrice = pls2;
                activeCycle = Cycle2;
                ...
                const discountPercent = Math.round(((pls - pls2) / pls) * 100);
                if (document.getElementById('how')) {
                    
                    document.getElementById('cup').style.display = "block";
                    coupnbosh.style.display = "none";
                    document.getElementById('app').style.display = "none";
                    document.getElementById('how').style.display = "block";
                    document.getElementById('how').innerHTML = `<span style="color:green;">Congratulations! Previous Cycle Student Discount Applied: ${discountPercent}% OFF!</span>`;
                }
        ...
        console.error('Error checking previous purchases:', error);
        return false;
    }
}
...
firebase.auth().onAuthStateChanged(function(e) {
    if (e) {
        ...
        var requestOptions = {
         ...
         fetch(`https://${shopName2}/v3/purchase/multiple/${Cycle}`, requestOptions)
            .then(response => {
            ...
            .then(async result => {
                if (result.status === 200) {
                    swal({
                        ...
                        return location.replace(result.invoices[0].invoice);
                    })
                } else {
                    // Check for previous purchases before enabling purchase
                    await checkPreviousPurchases(e.uid);
                    ...
                    form.addEventListener('submit', em => {
                        ...
                        fetch(`https://${shopName2}/${activeCycle}/${activeProductCode}/init`, requestOptions)
                            .then(response => {
                ...
                // Check for previous purchases in catch block as well
                await checkPreviousPurchases(e.uid);
...
cpn.addEventListener('click', (e) => {
    ...
    cupV.disabled = true;
    cpn.disabled = true;
    fetch(cuponApi + '/' + cpnCode.toUpperCase() + '/' + activeProductCode)
        .then((res) => {
            return res.json();
        })
        .then((loadedData) => {
            if (loadedData.status === "success") {
                var nes = activePrice - loadedData.Off;
                ...
                cpn.disabled = true;
                var percent = Math.round(((parseInt(loadedData.Off) + (fix - activePrice)) / fix) * 100);
                document.getElementById('how').style.display = "block";
